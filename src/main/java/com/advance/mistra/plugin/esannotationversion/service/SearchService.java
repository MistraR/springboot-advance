...
import static com.advance.mistra.plugin.esannotationversion.service.GlobalConstant.NON_FLAG;
import static com.advance.mistra.plugin.esannotationversion.service.GlobalConstant.PAGE;
import static com.advance.mistra.plugin.esannotationversion.service.GlobalConstant.SORT;
import static com.advance.mistra.plugin.esannotationversion.service.GlobalConstant.SPLIT_FLAG_COMMA;
import static com.advance.mistra.plugin.esannotationversion.service.GlobalConstant.WILDCARD;
...
public class SearchService {
    ...
    private NativeSearchQueryBuilder buildSearchField(Map<String, String> params, String indexName, String type,
        ...
        int size = Integer.parseInt(params.getOrDefault(GlobalConstant.SIZE, "10"));
        AtomicBoolean matchSearch = new AtomicBoolean(false);
        String q = params.get(GlobalConstant.Q);
        String missingFields = params.get(GlobalConstant.MISSING);
        String existsFields = params.get(GlobalConstant.EXISTS);
        ...
        NativeSearchQueryBuilder searchQueryBuilder = new NativeSearchQueryBuilder()
                .withIndices(params.getOrDefault(GlobalConstant.INDEX_NAME, indexName))
                .withTypes(params.getOrDefault(GlobalConstant.INDEX_TYPE, type))
                .withPageable(PageRequest.of(page, size));
        String fields = params.get(GlobalConstant.FIELDS);
        if (Objects.nonNull(fields)) {
            searchQueryBuilder.withFields(fields.split(SPLIT_FLAG_COMMA));
        }
        keyMappingsMap.getOrDefault(params.getOrDefault(GlobalConstant.INDEX_NAME, indexName), keyMappings)
                .entrySet()
                .stream()
                .filter(m -> m.getValue().getQueryType() == EsQueryTypeEnum.FULLTEXT
                        || m.getValue().getQueryType() != EsQueryTypeEnum.IGNORE
                        && params.get(m.getKey().toString()) != null)
                .forEach(m -> {
                    String k = m.getKey().toString();
                    FieldDefinition v = m.getValue();
                    String queryValue = params.get(k);
                    EsQueryTypeEnum queryType = v.getQueryType();
                    ...
                    switch (queryType) {
                        ...
                        case SHOULD:
                            ...
                            break;
                        case FULLTEXT:
                            if (!GlobalConstant.Q.equalsIgnoreCase(queryName)) {
